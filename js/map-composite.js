...
function app2() {
    queue()
    //should be town polym data
    .defer(d3.json, "js/data/town_yr_01geo.json")
    //.defer(d3.json, "us_states_5m.geo.json")
    //should be opioid death data
    .defer(d3.json, "js/data/town_yr_01geo.json")
    //.defer(d3.csv, "us_pop.csv")
...
function generateMap2(error, results) {
    ...
    svgContainer = d3.select("#map2").classed("svg-container", true) //container class to make it responsive
    .append("svg")
        .attr("preserveAspectRatio", "xMinYMin meet")
        .attr("viewBox", "0 0 " + mapWidth2 + " " + mapHeight2)
    //class to make it responsive
    .classed("svg-content-responsive", true)
        .style("border", "0px solid steelblue");
    ...
    opChgScale = d3.scale.threshold()
        .domain([0.01, 1.01])
        .range(["#bdbdbd", "#636363", "#6baed6", "#4292c6", "#2171b5", "#084594"]);
    opChgScale.domainStrings = function() {
        return (['0', '1', '2-5', '6-15', '16-33', '34-64', '>64']);
    };
    ...
    var projection2 = d3.geo.conicConformal()
        ...
        .rotate([71 + 30 / 60, -41])
        .scale([5000])
        .translate([mapWidth2 / 2.05, mapHeight2 * 1.05]);
    ...
    svgContainer.selectAll("path")
        ...
        var style = 'left:' + ((100 * mouse[0] / mapWidth2) + 3) + '% !important;top:' + (100 * mouse[1] / mapHeight2) + '% !important;';
        // Add transformation based on where the mouse is (top, bottom, left, right) to keep it in view
        var transformX = mouse[0] > mapWidth2 / 2 ? '-160%' : 0,
        transformY = mouse[1] > mapHeight2 / 2 ? '-100%' : 0;
        style += 'transform: translate(' + transformX + ',' + transformY + ');';
